...
def compute_embeddings(loader, model, do_normalize=False):
    ...
    for idx, mbatch in tqdm(enumerate(loader), total=len(loader)):
        ...
        embed = avalanche_forward(model, images, mbatch[-1])
        if do_normalize:
...
class KNNProbingAccuracyMetric(GenericPluginMetric[float, Accuracy]):
    def __init__(
        self, 
        train_stream, 
        test_stream, 
        ...
        self.knn_acc_k = dict()
        for k_i in self.k:
            self.knn_acc_k[k_i] = TaskAwareAccuracy()
        ...
        for k_i, acc in self.knn_acc_k.items(): # NOTE: v is an Accuracy() instance
            ...
            if isinstance(metric_value, dict):
                for k, v in metric_value.items():
                    ...
                    metric_name = metric_name.split("/")[0]+"_Acc/"+"/".join(metric_name.split("/")[1:]) + f"/k_{k_i}"
    ...
    def before_eval(self, strategy):
        ...
        with torch.enable_grad(): # NOTE: This is necessary because avalanche has a hidden torch.no_grad() in eval context!
            ...
            if isinstance(strategy.model.train_classifier, MultiTaskModule): # NOTE: this adds classifiers for every task possible
            ...
            #     if(isinstance(strategy.model.train_classifier, MultiTaskModule)):
    ...
    def update(self, strategy=None):
        ...
        for k_i in self.k:
            if isinstance(strategy.model.train_classifier, MultiTaskModule): # NOTE: this is copied from the 'avalanche_forward'-function 
                out = self.knn_classifier[task_label][k_i].predict(x_rep)
            else:  # no task labels
                out = self.knn_classifier[k_i].predict(x_rep)
            out = torch.from_numpy(out).to(strategy.device)
