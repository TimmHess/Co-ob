...
class ContinualEvaluationPhasePlugin(BaseSGDPlugin):
    ...
    def before_training(self, strategy: 'BaseStrategy', **kwargs):
        ...
        # Check for correct, necessary model type
        assert isinstance(strategy.model, FeatureExtractorModel)
        # Check whether the model has the correct method
        if not hasattr(strategy.model, 'get_features'):
            # Inject the function
            def get_features(obj, x, task_labels=None):
                x = avalanche_forward(obj.feature_extractor, x, task_labels)
                #obj.features = x.clone().detach()
                return x
            strategy.model.__class__.get_features = get_features

        # Call init on plugins
    ...
    def restore_strategy_(strategy, prev_state, prev_training_modes):
        ...
        for name, layer in strategy.model.named_modules():
            try:
                ...
                print("WARNING: Layer {} not found in previous training modes, skipping.".format(name))
