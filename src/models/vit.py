"""
Copyright by: https://github.com/s-chh/PyTorch-Vision-Transformer-ViT-MNIST-CIFAR10
"""
...
class EmbedLayer(nn.Module):
    def __init__(self, n_channels, embed_dim, image_size, patch_size, dropout=0.0):
        ...
        # Old learnable position embedding
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1,(image_size // patch_size) ** 2 + 1, embed_dim), requires_grad=True)
        # Sinosoidal position embedding (fixed)
        #self.pos_embedding = nn.Parameter(torch.tensor(get_positional_embeddings((image_size // patch_size) ** 2 + 1, embed_dim)), requires_grad=False)
...
class VisionTransformerFeat(nn.Module):
    def __init__(self, n_channels, embed_dim, n_layers, n_attention_heads, 
                 forward_mul, image_size, patch_size, n_classes=100, dropout=0.0):
        super().__init__()
